# COMPLETE FILE: src/auth.py
# Replace the entire file with this content

"""
TradeFlow NG — Authentication Module
Admin + Agent authentication with secure credential handling.
"""
...
def _get_admin_credentials():
    """
    Retrieve admin credentials from multiple sources.
...
def require_admin_login():
    """
    Call at top of app.py BEFORE any other content.
    Returns True if authenticated, False if showing login form.
    """
    ...
    with col2:
        ...
        username = st.text_input("Username", placeholder="admin", key="auth_username_input")
        password = st.text_input("Password", type="password", placeholder="Enter your password", key="auth_password_input")

        login_clicked = st.button("Login →", type="primary", use_container_width=True, key="auth_login_button")
...
def require_agent_login():
    """
    Call at top of agent_app.py BEFORE any other content.
    ...
    with col2:
        ...
        login_clicked = st.button("Login →", type="primary", use_container_width=True, key="agent_login_button")
        ...
        if login_clicked:
            ...
            if not phone_clean:
                ...
                # Query agents table with proper SQL syntax
                agent = _query(
                ...
                if not agent.empty:
                    ...
                    st.session_state["agent_state"] = (
                        str(state_row.iloc[0]["name"]) if not state_row.empty else "—"
                    )
                    ...
                    st.error("❌ Phone number not found. Contact your supervisor to get registered.")
...
def agent_logout():
    ...
    for key in ["agent_authenticated", "agent_id", "agent_name", "agent_state", "agent_state_id"]:
